...
if __name__ == "__main__":
    """Example usage"""
    # Initialize parser
    parser = WandbDataParser('scripts/exps/finegrained/sokoban/grounding_worldmodeling/wandb/offline-run-20250628_034929-pcf18dgl/wandb_sync_output.txt')
    ...
    if train_success or val_success:
        ...
        # 收集train success的所有values
        for metric in train_success:
            try:
                df = parser.get_metric_over_time(metric)
                all_train_values.extend(df['value'].tolist())
            except Exception as e:
                print(f"Error processing {metric}: {e}")

        # 收集val success的所有values  
        for metric in val_success:
            try:
                df = parser.get_metric_over_time(metric)
                all_val_values.extend(df['value'].tolist())
            except Exception as e:
                print(f"Error processing {metric}: {e}")
        ...
        print("\n📊 VAL VALUES LIST:")
        print(all_val_values)

        # 如果你想要所有values在一个列表里
        all_values = all_train_values + all_val_values
        print("\n📊 ALL VALUES:")
        print(all_values)
    else:
